...
int calculoBatalha(PERSONAGEM personagem, MONSTRO monstro, int monstroAtacante) {
    ...
    if (monstroAtacante == 1) {
        if (d20 + calcularModificador(monstro.forca) > calcularModificador(personagem.defesa)) {
        ...
        if (d20 + calcularModificador(personagem.forca) > calcularModificador(monstro.defesa)) {
            //dano de ataque é igual a 2d6 + modificador de ataque
            primeiroD6 = (rand() % 6) + 1;
            printf("Primeiro d6 rolado: %d\n",primeiroD6);
            segundoD6 = (rand() % 6) + 1;
