...
MONSTRO consultarNomeMonstroPorID(int id) {
    FILE *arquivo;
    char textoLido[200];
    char atributoMonstro[50];
    char valorAtributo[50];
    int contaVirgula, ultimaPosicao, contador = 0, contaMonstro = 0;
    arquivo = fopen("monstros.txt", "r");
    if (arquivo == NULL) {
        printf("Nao foi possivel abrir arquivo.");
    } else {
        ...
        while (fgets(textoLido, 200, arquivo) != NULL) {
            ...
            if(listaMonstro[contaMonstro].id == id){
                fclose(arquivo);
                return listaMonstro[contaMonstro];
            }
            contaMonstro++;
        }
        fclose(arquivo);
